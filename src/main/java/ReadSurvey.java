...
/**
 * Llegeix un fitxer CSV i converteix cada línea en una pregunta d'una enquesta. Cada pregunta ve amb el seu tipus, com s'indica a continuació
 *
 * Tipus de dades:
 *  0 → numèrica (Double)
...
public class ReadSurvey {
    ...
    public ReadSurvey(File filename, int opt) {
        this.filename = filename;
        this.opt = opt;
    }

    public void initializeVariableType(int[] type) {
        this.type = type;
    }
    ...
    public Object[][] readFile() {
        try (BufferedReader br = new BufferedReader(new FileReader(filename))) {
            ...
            this.type = new int[numCols-2];
            for (int i = 0; i < numCols-2; i++) {
                this.type[i] = Integer.parseInt(secondRow[i+1].trim());
            }
            ...
            this.data = new Object[numRows][this.type.length];
    ...
    public Object[][] processData(Object[][] data) {
        this.data = data;
        Object[] defaultValues = new Object[this.type.length];
        double[] maxValues = new double[this.type.length];
