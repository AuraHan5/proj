...
public class CalculateDistance {
    ...
    private static double singleChoiceOrdered(Object point, Object centroid) {
        ...
        double diff = pVal - cVal;
        double normalized = diff / (numOptions - 1);
    ...
    private static double freeText(Object point, Object centroid) {
        String strPoint = (String) point;
        String strCentroid = (String) centroid;

        if (strPoint.isEmpty() && strCentroid.isEmpty()) return 0.0;
        if (strPoint.isEmpty() || strCentroid.isEmpty()) return 1.0;

        double[] embPoint = WordEmbeddings.getEmbedding(strPoint);
        double[] embCentroid = WordEmbeddings.getEmbedding(strCentroid);

        double res = 1.0 - AuxiliarMethods.cosineSimilarity(embPoint, embCentroid);
