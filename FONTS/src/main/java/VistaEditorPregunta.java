...
public class VistaEditorPregunta extends JPanel {
    ...
    public VistaEditorPregunta(CtrlPresentacio ctrl, 
                               String enunciat, 
                               String tipus, 
        ...
        if (opcionsSerialized != null && !opcionsSerialized.isEmpty()) {
            // Split options using the dedicated options separator
    ...
    private void inicialitzarComponents(String enunciatInicial, String tipusInicial, Integer maxRespostesInicial) {
        ...
        // Spinner personalitzat per maxRespostes: permet -1 o valors des de 2 cap amunt
        SpinnerNumberModel spinnerModel = new SpinnerNumberModel(-1, -1, Integer.MAX_VALUE, 1) {
            @Override
            public Object getNextValue() {
                int current = ((Number) getValue()).intValue();
                if (current == -1) return 2; // De -1 salta a 2
                int max = ((Number) getMaximum()).intValue();
            ...
            public Object getPreviousValue() {
                int current = ((Number) getValue()).intValue();
                if (current == 2) return -1; // De 2 salta a -1
                if (current > 2) return current - 1;
                return current;
            }
        };
        
        spinnerMaxRespostes = new JSpinner(spinnerModel);
        ...
        btnEliminar.addActionListener(e -> {
            if (indexPregunta < 0) {
                JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(this), "No es pot eliminar una pregunta que encara no existeix.", "Error", JOptionPane.WARNING_MESSAGE);
                return;
            }
    ...
    private void eliminarUltimaOpcio() {
        if (llistaTextFields.size() > 1) {
            ...
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(this), "Hi ha d'haver com a mínim una opció disponible.");
        }
    }
    ...
    private void updateSpinnerLimit() {
        ...
        // Establir mínim a -1 per permetre "sense límit"
        ...
        try {
            ...
            // Si el valor actual és entre 0 i 1, ajustar-lo a 2 (mínim vàlid diferent de -1)
            if (cur >= 0 && cur < 2) {
                model.setValue(Integer.valueOf(2));
    ...
    private boolean guardarDadesCurrents() {
        ...
        if (nouEnunciat.isEmpty()) {
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(this), "L'enunciat no pot estar buit.", "Error", JOptionPane.WARNING_MESSAGE);
            return false;
        }
        ...
        if (!esPreguntaText) {
            for (JTextField txt : llistaTextFields) {
                ...
                if (valor.isEmpty()) {
                    JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(this), "No pots deixar opcions buides.", "Error", JOptionPane.WARNING_MESSAGE);
                    return false;
                }
                novesOpcions.add(valor);
            }
            if (novesOpcions.isEmpty()) {
                JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(this), "Has d'afegir almenys una opció.", "Error", JOptionPane.WARNING_MESSAGE);
                return false;
            }
        } 
        ...
        if (tipusSeleccionat != null && tipusSeleccionat.contains("Múltiple")) {
            try { 
                max = (Integer) spinnerMaxRespostes.getValue();
                // Validar que sigui -1 o >= 2
                if (max != -1 && max < 2) {
                    JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(this), "El màxim de respostes ha de ser -1 (sense límit) o mínim 2.", "Error", JOptionPane.WARNING_MESSAGE);
                    return false;
                }
                // Només validar si max != -1 (si té límit)
                if (max != -1 && max > novesOpcions.size()) {
                    JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(this), "El màxim de respostes no pot ser més gran que el nombre d'opcions (" + novesOpcions.size() + ").", "Error", JOptionPane.WARNING_MESSAGE);
                    return false;
                }
            } catch (Exception ex) { 
                max = -1; 
